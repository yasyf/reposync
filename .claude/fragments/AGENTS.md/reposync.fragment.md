...
**Testing.** Tests live beside the code they cover as `*_test.go` files. Run the whole suite with `go test ./...` (CI runs it with `-race`). The `internal/sync` tests spin up real ephemeral git repos in temp dirs rather than mocking git.
